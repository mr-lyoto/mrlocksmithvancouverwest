...
class View
{
	...
	protected $viewEngineMap = array(
		'.tmpl.php' => 'WPSFramework\\Mvc\\Engine\\BladeEngine',
		'.php'       => 'WPSFramework\\Mvc\\Engine\\PhpEngine',
	);
	 ...
	 * @param   callable  $callback     A method to post-process the view template (I use leetspeak here because of bad quality hosts with broken scanners)
	 *
	...
	protected function getExtension($path)
	{
		...
		return wpspeed_array_first($extensions, function($key, $value) use ($path)
		{
			return wpspeed_ends_with($path, $value);
		});
	}
	 ...
	 * @param  array  $data         The array of data you want to render
	...
	public function renderEach($viewTemplate, $data, $eachItemName, $empty = 'raw|')
	{
		...
		{
			if (wpspeed_starts_with($empty, 'raw|'))
			{
				$result = substr($empty, 4);
			}
			elseif (wpspeed_starts_with($empty, 'text|'))
			{
	 ...
	 *
	 * @return \WPSFramework\Container\Container
	 */
