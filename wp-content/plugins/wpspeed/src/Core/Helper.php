...
class Helper
{
	...
	{
		WPSPEED_DEBUG ? Profiler::start( 'MinifyHtml' ) : null;


		if ( $oParams->get( 'combine_files_enable', '1' ) && $oParams->get( 'html_minify', 1 ) )
		{
			$aOptions = array();

			if ( $oParams->get( 'css_minify', 1 ) )
			{
				$aOptions['cssMinifier'] = array( 'WPSpeed\Minify\Css', 'optimize' );
			}

			if ( $oParams->get( 'js_minify', 1 ) )
			{
				$aOptions['jsMinifier'] = array( 'WPSpeed\Minify\Js', 'optimize' );
			}

			$aOptions['jsonMinifier'] = array( 'WPSpeed\Minify\Json', 'optimize' );
			...
			WPSPEED_DEBUG ? Profiler::stop( 'MinifyHtml', true ) : null;
		}
	...
	public static function postAsync( $url, $params, array $posts )
	{
		...
		if ( ! $fp )
		{
			...
			Logger::debug( $errno . ': ' . $errstr, 'WPSPEED_post-error' );
		}
		else
		{
