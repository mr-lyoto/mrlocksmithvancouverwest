...
if (!function_exists('fairy_alter_excerpt')) :
    ...
    function fairy_alter_excerpt($length)
    {
        ...
        global $fairy_theme_options;
        if (!empty($fairy_theme_options['fairy-excerpt-length'])) {
            return absint($fairy_theme_options['fairy-excerpt-length']);
        }
        return 25;
    }
endif;
...
{
    wp_enqueue_style('fairy_customizer_fonts', 'https://fonts.googleapis.com/css?family=Libre+Baskerville|Merriweather|Montserrat|Muli|Mulish|Open+Sans|Open+Sans+Condensed|Oswald|Oxygen|Poppins|Roboto|Voltaire|Yanone+Kaffeesatz', array(), null);
}
